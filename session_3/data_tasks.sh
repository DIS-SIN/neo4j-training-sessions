...
if [ $# -lt 2 ]; then
  ...
  echo "  EXAMPLES:"
  echo "      ./data_tasks.sh ../../neo4j-algo-apoc n: normalize registration & survey data only"
  echo "      ./data_tasks.sh ../../neo4j-algo-apoc e: extract entities and relationships only"
  echo "      ./data_tasks.sh ../../neo4j-algo-apoc p: set initial password for neo4j only"
  echo "      ./data_tasks.sh ../../neo4j-algo-apoc i: import data only"
  echo "      ./data_tasks.sh ../../neo4j-algo-apoc b: backup the graph database only"
  echo "      ./data_tasks.sh ../../neo4j-algo-apoc nepib: run a n -> e -> p -> i -> b pipeline (1ST TIME)"
  echo "      ./data_tasks.sh ../../neo4j-algo-apoc ib: run a i -> b pipeline (fresh IMPORT)"
  echo " NOTE: order of (optional) execution is n -> e -> p -> i -> b"
  exit
fi
